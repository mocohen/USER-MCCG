  ...
  detailedTimers(false)
{
  ...
  word_map["setEngineArgs"]=SETENGINEARGS;
}
  ...
  if(nw==1) {
    switch(word_map[word]) {
      ...
      case GETBIAS:
        ...
        break;
      case SETENGINEARGS:
        //plumed_merror("set engine args");
        log.printf("Set Engine Args!\n");
        log.flush();
        CHECK_INIT(initialized,word);
        CHECK_NULL(val,word);
        atoms.setEngineArgs(val);
        break;        
      default:
...
// check that initialization just happens once
  ...
  log<<"PLUMED is starting, and something stupid\n";
...
log.printf("MCCG: Just calculate\n");  
log.flush();
  if(!active)return;
  ...
  int iaction=0;
  log.printf("Loop through actions\n"); 
// calculate the active actions in order (assuming *backward* dependence)
  for(ActionSet::iterator p=actionSet.begin();p!=actionSet.end();++p){
    log.printf("MCCG: New actions\n");
    log.flush();
  ...
  log.printf("Finished looping through actions\n");
  log.flush(); 
  stopwatch.stop("4 Calculating (forward loop)");
}

void PlumedMain::justApply(){
  log.printf("MCCG: Just Apply\n\n\n");  
  log.flush();  
  if(!active)return;
  int iaction=0;
...
// apply them in reverse order
  for(ActionSet::reverse_iterator p=actionSet.rbegin();p!=actionSet.rend();++p){
    ...
    if((*p)->isActive()){
      ...
      log.printf("\n");
      log.flush();
      log.printf("MCCG: B Apply\n");
      log.flush();
      (*p)->apply();
      log.printf("MCCG: After Apply\n");
      log.flush();
...
// still ActionAtomistic has a special treatment, since they may need to add forces on atoms
      if(!a){       log.printf("MCCG: A Not Exists\n"); log.flush();}
      else{       log.printf("MCCG: A Exists\n"); log.flush();}

      if(a) a->applyForces();
      EArgs::PassEngineArgs*b=dynamic_cast<EArgs::PassEngineArgs*>(*p);
      if(b){
        log.printf("MCCG: A exists\n");
        log.flush();
        a->applyEngineArgs();
      }
      //else plumed_merror("not a");
      
      if(detailedTimers) stopwatch.stop(actionNumberLabel);
    }
    iaction++;
  }
  log.printf("MCCG: Done looping through actions\n");
  log.flush();  
// this is updating the MD copy of the forces
  ...
  if(atoms.getEngineArgsSize()>0) atoms.updateEngineArgs();
