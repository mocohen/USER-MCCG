...
void ActionAtomistic::applyForces(){
  ...
  log.printf("MCCG: Done Apply Forces\n");
  log.flush();

}

//***** MCCG
void ActionAtomistic::applyEngineArgs(){
  log.printf("MCCG: Apply EngineArgs Action Atomistic\n");
  log.flush();
  //EArgs::PassEngineArgs*a=dynamic_cast<EArgs::PassEngineArgs*>(p);
  if(doEngineArgs){
   vector<double> & eA(atoms.engineArgs);
   log.printf("MCCG: size eA %d engineArgs %d\n", eA.size(), engineArgs.size());
   log.flush();
   for(int i=0; i < engineArgs.size(); i++){
   	 eA[i] = engineArgs[i];
   }
  }
}
