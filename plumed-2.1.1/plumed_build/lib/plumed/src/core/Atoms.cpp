...
void Atoms::setEngineArgs(void*p){
 plumed_massert( dataCanBeSet , "setEngineArgs must be called after setStep in MD code interface");
 mdatoms->setEArgs(p); engineArgsHaveBeenSet=true;
...
//***** MCCG
void Atoms::updateEngineArgs(){
  printf("update Engine args in Atoms.cpp");
  plumed_error();
  //error("Into update Engine Args in Atoms.cpp");
  plumed_assert(engineArgsHaveBeenSet);
  mdatoms->updateEArgs(engineArgs);
}
...
//***** MCCG
void Atoms::initEngineArgs(int nEngineArgs){
  engineArgs = std::vector<double>(nEngineArgs);
}
