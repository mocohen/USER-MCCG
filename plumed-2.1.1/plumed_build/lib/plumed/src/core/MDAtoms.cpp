...
//***** MCCG  
  //void getEngineArgs(const vector<int>&index,vector<double>&)const;
  void updateEArgs(const vector<double>&eArgs);
//***** END MCCG  
...
/*template <class T>
void MDAtomsTyped<T>::getEngineArgs(const vector<int>&index,vector<double>&engineArgs)const{
  if(eA) for(unsigned i=0;i<index.size();++i) engineArgs[index[i]]=eA[i];
  else  for(unsigned i=0;i<index.size();++i) engineArgs[index[i]]=0.0;
}*/
...
//***** MCCG
template <class T>
void MDAtomsTyped<T>::updateEArgs(const vector<double>&eArgs){
...
template <class T>
void MDAtomsTyped<T>::setEArgs(void*eA){
  this->eA=static_cast<T*>(eA);
}
